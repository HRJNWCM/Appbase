 ...
 * <p/>
 * Remember to call the {@link #stopJumping()} method once you're done using the
 * JumpingBeans (that is, when you detach the TextView from the view tree, you
 * hide it, or the parent Activity/Fragment goes in the paused status). This
 * will allow to release the animations and free up memory and CPU that would be
 * otherwise wasted.
 * <p/>
 * Please note that you:
 * <ul>
 * <li><b>Must not</b> try to change a jumping beans text in a textview before
 * calling {@link #stopJumping()} as to avoid unnecessary invalidation calls;
 * the JumpingBeans class cannot know when this happens and will keep animating
 * the textview (well, try to, anyway), wasting resources</li>
 * <li><b>Must not</b> try to use a jumping beans text in another view; it will
 * not animate. Just create another jumping beans animation for each new view</li>
 * <li><b>Must not</b> use more than one JumpingBeans instance on a single
 * TextView, as the first cleanup operation called on any of these JumpingBeans
 * will also cleanup all other JumpingBeans' stuff. This is most likely not what
 * you want to happen in some cases.</li>
 * <li><b>Should not</b> use JumpingBeans on large chunks of text. Ideally this
 * should be done on small views with just a few words. We've strived to make it
 * as inexpensive as possible to use JumpingBeans but invalidating and possibly
 * relayouting a large TextView can be pretty expensive.</li>
 * </ul>
 */
public final class JumpingBeans
{
	 ...
	 * animating. The rest of the range will be spent in "resting" state. This
	 * the "duty cycle" of the jumping animation.
	 */
	...
	private static CharSequence removeJumpingBeansSpans(Spanned text)
	{
		...
		for ( Object span : spans)
		{
			...
			{
				sbb.setSpan(span, text.getSpanStart(span), text
				        .getSpanEnd(span), text.getSpanFlags(span));
			}
		}
		return sbb;
	}
	...
	public static class Builder
	{
		 ...
		 * <p/>
		 * If the TextView has no text, the resulting TextView text will consist
		 * of the three dots only.
		 * <p/>
		 * The TextView text is cached to the current value at this time and set
		 * again in the {@link #build()} method, so any change to the TextView
		 * text done in the meantime will be lost. This means that <b>you should
		 * do all changes to the TextView text <i>before</i> you begin using
		 * this builder.</b>
		 * <p/>
		 ...
		 * 
		 * @param textView
		 *            The TextView to append the dots to
		...
		public Builder appendJumpingDots(TextView textView)
		{
			...
			CharSequence text = !TextUtils.isEmpty(textView.getText()) ? textView
			        .getText() : "";
			if (text.length() > 0
			...
			if (text.length() < 3
			        || !TextUtils
			                .equals(text.subSequence(text.length() - 3, text
			                        .length()), "..."))
			{
		 ...
		 * 
		 * @param textView
		 *            The TextView whose text is to be animated
		 * @param startPos
		 *            The position of the first character to animate
		 * @param endPos
		 *            The position after the one the animated range ends at
		 ...
		 * animating. The rest of the time will be spent "resting". The default
		 * value is
		 * {@link net.frakbot.jumpingbeans.JumpingBeans#DEFAULT_ANIMATION_DUTY_CYCLE}
		 * .
		 * 
		 * @param animatedRange
		 *            The fraction of the animation loop time spent actually
		 ...
		 * 
		 * @param loopDuration
		 *            The jumping animation loop duration, in milliseconds
		 */
		 ...
		 * the start of the previous one, in milliseconds. The default value is
		 * the loop length divided by three times the number of character
		 * animated by this instance of JumpingBeans.
		 * <p/>
		 * Only has a meaning when the animation is a wave.
		 * 
		 * @param waveCharOffset
		 ...
		 * (i.e., with a delay between each other) or all at the same time.
		 * 
		 * @param wave
		 *            If true, the animation is going to be a wave; if false,
		 *            all characters will jump ay the same time
		...
		public JumpingBeans build()
		{
			...
			} else
			{
				...
				{
					JumpingBeansSpan jumpingBean = new JumpingBeansSpan(textView, loopDuration, pos
					        - startPos, waveCharDelay, animRange);
