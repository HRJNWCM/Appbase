...
public class WheelView extends View
{
	...
	private int getMaxTextLength()
	{
		...
		for ( int i = Math.max(currentItem - addItems, 0); i < Math
		        .min(currentItem + visibleItems, adapter.getItemsCount()); i++)
		{
	...
	private int calculateLayoutWidth(int widthSize, int mode)
	{
		...
		if (maxLength > 0)
		{
			float textWidth = (float)Math.ceil(Layout
					.getDesiredWidth("0", itemsPaint));
		...
		{
			labelWidth = (int)Math.ceil(Layout
			        .getDesiredWidth(label, valuePaint));
		}
	...
	private void createLayouts(int widthItems, int widthLabel)
	{
		...
		{
			String text = getAdapter() != null ? getAdapter()
			        .getItem(currentItem) : null;
	...
	private SimpleOnGestureListener	gestureListener	= new SimpleOnGestureListener()
	                                                {
		                                                ...
		                                                public boolean onFling(
		                                                        ...
		                                                        float velocityY)
		                                                {
			                                                lastScrollY = currentItem
			                                                        * getItemHeight()
			                                                        + scrollingOffset;
			                                                int maxY = isCyclic ? 0x7FFFFFFF : adapter
			                                                        .getItemsCount()
			                                                        * getItemHeight();
	...
	private Handler	animationHandler	= new Handler()
	                                 {
		                                 public void handleMessage(Message msg)
		                                 {
			                                 ...
			                                 if (Math.abs(currY
			                                         - scroller.getFinalY()) < MIN_DELTA_FOR_SCROLLING)
			                                 {
