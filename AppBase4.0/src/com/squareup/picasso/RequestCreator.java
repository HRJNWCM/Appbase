...
public class RequestCreator
{
	 ...
	 * <p>
	 * By default, Picasso will either set a supplied placeholder or clear the
	 * target {@link ImageView} in order to ensure behavior in situations where
	 * views are recycled. This method will prevent that behavior and retain any
	 * already set image.
	 */
	 ...
	 * this resource will be set on the target {@link ImageView}.
	 */
	 ...
	 * {@link ImageView}'s bounds. This will result in delayed execution of the
	 * request until the {@link ImageView} has been laid out.
	 * <p>
	...
	/**
	 * Only resize an image if the original image size is bigger than the target
	 * size specified by {@link #resize(int, int)}.
	 */
	 ...
	 * {@link BitmapFactory.Options#inPreferredConfig its documentation} for
	 * more details.
	 */
	...
	/**
	 * Sets the stable key for this request to be used instead of the URI or
	 * resource ID when caching. Two requests with the same value are considered
	 * to be for the same resource.
	 */
	 ...
	 * guarantee it. By default, all requests have {@link Priority#NORMAL}
	 * priority, except for {@link #fetch()} requests, which have
	 * {@link Priority#LOW} priority by default.
	 */
	...
	/**
	 * @deprecated Use {@link #memoryPolicy(MemoryPolicy, MemoryPolicy...)}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Specifies the {@link MemoryPolicy} to use for this request. You may
	 * specify additional policy options using the varargs parameter.
	 */
	...
	/**
	 * Specifies the {@link NetworkPolicy} to use for this request. You may
	 * specify additional policy options using the varargs parameter.
	 */
	 ...
	 * because the underlying {@link Cache} implementation is not guaranteed to
	 * be thread-safe.
	 */
	public Bitmap get() throws IOException
	{
		...
		return forRequest(picasso, picasso.dispatcher, picasso.cache, picasso.stats, action)
		        .hunt();
	}

	/**
	 * Asynchronously fulfills the request without a {@link ImageView} or
	 * {@link Target}. This is useful when you want to warm up the cache with an
	 * image.
	 * <p>
	...
	/**
	 * Asynchronously fulfills the request without a {@link ImageView} or
	 * {@link Target}, and invokes the target {@link Callback} with the result.
	...
	/**
	 * Asynchronously fulfills the request into the specified {@link Target}. In
	 * most cases, you should use this when you are dealing with a custom
	 * {@link android.view.View View} or view holder which should implement the
	 ...
	 * </blockquote>
	 * <p>
	 * <em>Note:</em> This method keeps a weak reference to the {@link Target}
	 * instance and will be garbage collected if you do not keep a strong
	 * reference to it. To receive callbacks when an image is loaded use
	 ...
	 * {@link RemoteViews} object with the given {@code viewId}. This is used
	 * for loading bitmaps into a {@link Notification}.
	 */
	        ...
	        Notification notification)
	{
		...
		if (placeholderDrawable != null || placeholderResId != 0
		        || errorDrawable != null)
		{
		...
		RemoteViewsAction action = new NotificationAction(picasso, request, remoteViews, viewId, notificationId, notification, memoryPolicy, networkPolicy, key, tag, errorResId);
	...
	public void into(RemoteViews remoteViews, int viewId, int[] appWidgetIds)
	{
		...
		Request request = createRequest(started);
		String key = createKey(request, new StringBuilder()); // Non-main thread needs own builder.
		RemoteViewsAction action = new AppWidgetAction(picasso, request, remoteViews, viewId, appWidgetIds, memoryPolicy, networkPolicy, key, tag, errorResId);
		performRemoteViewInto(action);
	}
	 ...
	 * {@link ImageView} instance and will automatically support object
	 * recycling.
	 */
	 ...
	 * <em>Note:</em> The {@link Callback} param is a strong reference and will
	 * prevent your {@link android.app.Activity} or {@link android.app.Fragment}
	 * from being garbage collected. If you use this method, it is
	 * <b>strongly</b> recommended you invoke an adjacent
	...
	private Request createRequest(long started)
	{
		...
		if (transformed != request)
		{
			...
			if (loggingEnabled)
			{
				log(OWNER_MAIN, VERB_CHANGED, transformed.logId(), "into "
				        + transformed);
			}
		}
		return transformed;
